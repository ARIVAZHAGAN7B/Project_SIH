...
export class KolamExporter {
	static async exportAsSVG(pattern) {
	...
	static async downloadSVG(pattern, filename) {
	...
	static async exportAsPNG(element, options = { format: 'png' }) {
	...
	static async downloadPNG(element, filename, options = { format: 'png' }) {
	...
	static async exportAsAnimatedGIF(
		...
		for (let frame = 0; frame <= frameCount; frame++) {
			...
			allDots.forEach((dot, index) => {
				(dot).style.opacity = index < dotsToShow ? '1' : '0';
			});

			allCurves.forEach((curve, index) => {
				(curve).style.opacity = index < curvesToShow ? '1' : '0';
			});
	...
	static async getEmbedCode(pattern, options = {}) {
